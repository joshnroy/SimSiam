...
export CUDA_VISIBLE_DEVICES=0,1 && export WANDB_RUN_GROUP=byol_cifar_debug && python main.py --config_file="configs/byol_cifar.yaml" --data_dir="../stream_data" --log_dir="../logs/cifar-contrastive-logs-${WANDB_RUN_GROUP}-${ID}/" --ckpt_dir=".cache/${WANDB_RUN_GROUP}" --linear_monitor --temporal_jitter_range=0 --preload_dataset --download --wandb
