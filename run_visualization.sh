...
source ~/miniconda3/bin/activate && conda activate simsiam && python3 linear_eval.py --config_file="configs/simsiam_ucf101.yaml" --data_dir="/users/jroy1/data/jroy1/contrastive/ucfimages64x64" --log_dir="../logs/contrastive-logs-${exp_type}-${ID}/" --ckpt_dir=".cache/${exp_type}/" --linear_monitor --temporal_jitter_range=50 --download --preload_dataset --eval_from=".cache/simsiam_ucf101-cifar_time_jittering_deterministic50_staticaugs/simsiam-ucf101-experiment-resnet18_cifar_variant1_0630201817.pth" --small_dataset
